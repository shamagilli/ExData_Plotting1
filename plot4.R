...
# Date:             Dec 7 2014
# Run:              create a source and call the function: plot4 ()
# 
# Functionality:
...
leftup <- function (d = download() ) {
  with(d, plot(Time, Global_active_power, xlab="", ylab = "Global Active Power", bg="transparent", lty=1, lwd=1, pch="."))
  with(d, lines(Time, Global_active_power))
  
}

leftdown <- function (d = download() ) {
...
rightup <- function (d = download() ) {
  with(d, plot(Time, Voltage, xlab="datetime", ylab = "Voltage", bg="transparent", lty=1, lwd=1, pch="."))
  with(d, lines(Time, Voltage))
}

rightdown <- function (d = download() ) {
  with(d, plot(Time, Global_reactive_power, xlab="datetime",  bg="transparent", lty=1, lwd=1, pch="."))
  with(d, lines(Time, Global_reactive_power))
}

plot4 <- function (d = download() ) {
  file.plot <- "plot4.png"
